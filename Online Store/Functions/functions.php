...
function getCategoryProduct() {
    if(isset($_GET['cat'])) {
        ...
        if ($count_cats == 0) {
            echo "<h2>There is no Product Available in this Category!</h2> ";
        }
        else {
...
function getBrandProduct() {
    if(isset($_GET['brand'])) {
        ...
        global $con;
        $get_cat_pro = "select * from products where product_brand = '$brand_id'";
        $run_cat_pro = mysqli_query($con,$get_cat_pro);
        $count_cats = mysqli_num_rows($run_cat_pro);

        if ($count_cats == 0) {
            echo "<h2>There is no Product Available in this Brand!</h2> ";
        }
        else {
            while ($row_cat_pro=mysqli_fetch_array($run_cat_pro)){
